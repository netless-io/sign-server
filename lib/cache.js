...
export function cache_clear() {
  ...
  // NOTE: hard-coded files to be removed, built by electron builder
  for (const hash in meta.keep) {
    const name = meta.keep[hash];
    if (name.includes("__uninstaller") || name.includes("-x64-")) {
      const dir = path.join(cache_dir, hash);
      fs.rmSync(dir, { force: true, recursive: true });
      delete meta.keep[hash];
    }
  }
  meta.temp = {};
  meta.sha1 = {};
